...
{{PRODUCT_NAME}}（以下简称“我们”）是由武汉创意考拉科技有限公司运营的一款以智能系统与自动化能力为核心的平台产品。我们通过 {{PRODUCT_NAME}} 平台向用户提供相关服务。{{PRODUCT_NAME}} 不仅能够基于用户输入生成内容，还可以在用户授权的前提下代表用户执行操作、调用外部服务、访问计算资源或使用用户提供的凭据完成特定任务。

我们重视用户信息与数据安全。本隐私政策旨在向你说明，在你使用 {{PRODUCT_NAME}} 平台及相关服务（以下统称“服务”）过程中，我们如何收集、使用、存储、共享及保护你的信息。
...
在服务运行过程中，我们通过 {{PRODUCT_NAME}} 系统及其智能体可能会产生并记录以下信息：
...
* 通过 {{PRODUCT_NAME}} 系统根据你的明确指示执行任务和操作（包括代表你调用外部服务或系统）
* 存储系统运行记录、执行结果及历史状态
* 分析系统使用情况、性能与可靠性
...
你理解并同意：
**你在平台中提交的信息，以及我们通过 {{PRODUCT_NAME}} 系统在运行过程中生成、执行或产生的内容与行为结果，均可能被用于上述用途，包括模型训练和系统优化。**

## 四、自动化执行与授权说明

{{PRODUCT_NAME}} 是一款自动化智能系统。在你授权的范围内，我们可能通过 {{PRODUCT_NAME}} 系统代表你执行操作，包括但不限于：
